...
if (document.querySelector('.slider-sites__body')) {
  new Swiper('.slider-sites__body', {
    observer: true,
    ...
    preloadImage: false,
    navigation: {
      nextEl: '.slider-sites .slider-arrow_next',
      prevEl: '.slider-sites .slider-arrow_prev'
    },
    breakpoints: {
      1212: {
        slidesPerView: 3
...
if (spoilersArray.length > 0) {
  ...
  if (spoilersMedia.length > 0) {
    ...
    let mediaQueires = breakpointsArray.map(function (item) {
      return '(' + item.type + "-width: " + item.value + "px)," + item.value + ',' + item.type;
    });
    mediaQueires = mediaQueires.filter(function (item, index, self) {
      return self.indexOf(item) === index;
    }); //Работаем с каждым брекпоинтом

    mediaQueires.forEach(breakpoint => {
      const paramsArray = params.split(",");
      const mediaBreakpoint = paramsArray[1];
      const mediaType = paramsArray[1];
      const matchMedia = window.matchMedia(paramsArray[0]); //Объекты с нужным условием
  ...
  function setSpollerAction(e) {
    ...
    if (el.hasAttribute('data-spoller') || el.closest('[data-spoller]')) {
      ...
      const spollersBlock = spollerTitle.closest('[data-spoilers]');
...
window.onload = function () {
  ...
  const menuBtn = document.querySelector('.menu__bottom');
  ...
  if (menuBtn !== null) {
    menuBtn.addEventListener('click', function () {
      menuBtn.classList.toggle('mobile');
  ...
  function animateProgressBar() {
    const prBar = document.querySelector('.pr-bar__line');
    let prBarValSourse = +prBar.textContent.match(/\d+/);
    prBar.style.width = `0%`;
    let width = 0;
    let id = setInterval(frame, 20);

    function frame() {
      if (width >= prBarValSourse) {
        clearInterval(id);
      } else {
        width++;
  ...
  if (document.querySelector('.module.company')) {
    ...
    /* Подстановка валюты в input */


    currency.map(function (el) {
      el.addEventListener('change', () => {
        el.value.match(/[0-9]*/)[0] ? el.value = el.value.match(/[0-9]*/) + '₽' : el.value = '';
      });
    });
...
$(function () {
  ...
  function closeModal() {
    ...
    onScroll();
  } //Блокировка body при открытии модалки
  ...
  } //Снятие блокировки body при закрытии модалки
  ...
  $(document).on('click', '.auth__icon', function () {
    ...
    if (input.attr('type') == 'password') {
      $(this).removeClass('ic_hide');
      $(this).addClass('ic_show');
      input.attr('type', 'text');
    } else {
      $(this).removeClass('ic_show');
      $(this).addClass('ic_hide');
  ...
  $(document).on('click', '#auth-system', function () {
    ...
    $(this).parents('.auth__form').find('.auth__btn').css({
      'opacity': '.5',
      'pointer-events': 'none'
    });
    $(this).parents('.auth__form').next().css({
      'opacity': '.5',
      'pointer-events': 'none'
    });
  });
